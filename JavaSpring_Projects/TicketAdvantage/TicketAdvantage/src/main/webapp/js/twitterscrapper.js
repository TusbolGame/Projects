...
$.fn.setupTwitterAccountSelect = function(scrapper) {
	...
	$.fn.serviceCall('GET', '', 'restapi/twitteraccount/userid/' + uId, 45000, function(data) {
	    ...
	    for (var i=0; (typeof data != 'undefined') && (data != null) && (i < data.length); ++i) {
		    	...
		    	liString.push('  <input id="src' + data[i].id + '" onclick="$.fn.determineTwitterSourceAction(' + scrapper.id + ', ' + data[i].id + ');" type="checkbox" value="' + data[i].id + '" /><span class="acctdivspan">' + data[i].name + '</span>');	
		    	liString.push('</div>');
	    }
		$('#source-account-list').html(liString.join('\n'));

		// Now map the source, destination and middle (if any)
		$.fn.mapTwitterSourceAccts(scrapper.sources);
	});
	...
	$.fn.serviceCall('GET', '', 'restapi/account/userid/' + uId, 45000, function(data) {
	    ...
	    for (var i=0; (typeof data != 'undefined') && (data != null) && (i < data.length); ++i) {
		    	liString.push('<div id="acctdiv' + data[i].id + '" class="acctdiv">');
		    	liString.push('  <input id="dest' + data[i].id + '" onclick="$.fn.determineTwitterDestinationAction(' + scrapper.id + ', ' + data[i].id + ');" type="checkbox" value="' + data[i].id + '" /><span class="acctdivspan">' + data[i].name + '</span>');	
		    	liString.push('</div>');
	    }
		$('#destination-account-list').html(liString.join('\n'));
		$.fn.mapTwitterDestinationAccts(scrapper.destinations);

		liString = [];
		// Loop through all of the accounts
	    for (var i=0; (typeof data != 'undefined') && (data != null) && (i < data.length); ++i) {
		    	liString.push('<div id="acctdiv' + data[i].id + '" class="acctdiv">');
		    	liString.push('  <input id="mid' + data[i].id + '" onclick="$.fn.determineTwitterMiddleAction(' + scrapper.id + ', ' + data[i].id + ');" type="checkbox" value="' + data[i].id + '" /><span class="acctdivspan">' + data[i].name + '</span>');	
		    	liString.push('</div>');
	    }
		$('#middle-account-list').html(liString.join('\n'));
		$.fn.mapTwitterMiddleAccts(scrapper.twittermiddledestinations);

		liString = [];
		// Loop through all of the accounts
	    for (var i=0; (typeof data != 'undefined') && (data != null) && (i < data.length); ++i) {
		    	liString.push('<div id="acctdiv' + data[i].id + '" class="acctdiv">');
		    	liString.push('  <input id="order' + data[i].id + '" onclick="$.fn.determineTwitterOrderAction(' + scrapper.id + ', ' + data[i].id + ');" type="checkbox" value="' + data[i].id + '" /><span class="acctdivspan">' + data[i].name + '</span>');	
		    	liString.push('</div>');
	    }
		$('#order-account-list').html(liString.join('\n'));
		$.fn.mapTwitterOrderAccts(scrapper.twitterorderdestinations);

		liString.length = 0;
	});
}

$.fn.determineTwitterSourceAction = function(id, accountid) {
	if ($('#src' + accountid).is(':checked')) {
		$.fn.addTwitterSource(id, accountid);
	} else {
		$.fn.deleteTwitterSource(id, accountid);
	}
}

$.fn.determineTwitterDestinationAction = function(id, accountid) {
	if ($('#dest' + accountid).is(':checked')) {
		$.fn.addTwitterDestination(id, accountid);
	} else {
		$.fn.deleteTwitterDestination(id, accountid);
	}
}

$.fn.determineTwitterMiddleAction = function(id, accountid) {
	if ($('#mid' + accountid).is(':checked')) {
		$.fn.addTwitterMiddle(id, accountid);
	} else {
		$.fn.deleteTwitterMiddle(id, accountid);
	}	
}

$.fn.determineTwitterOrderAction = function(id, accountid) {
	if ($('#order' + accountid).is(':checked')) {
		$.fn.addTwitterOrder(id, accountid);
	} else {
		$.fn.deleteTwitterOrder(id, accountid);
	}	
}

$.fn.mapTwitterSourceAccts = function(data) {
...
$.fn.mapTwitterDestinationAccts = function(data) {
...
$.fn.mapTwitterMiddleAccts = function(data) {
...
$.fn.mapTwitterOrderAccts = function(data) {
...
//Scrapper Listings
$.fn.getTwitterScrapperListings = function(id) {
	$.fn.serviceCall('GET', '', 'restapi/twitterscrapper/id/' + id, 45000, function(data) {
    		// Get the account listings
		$.fn.setupTwitterAccountSelect(data);
...
$.fn.addTwitterSource = function(id, accountid) {
	$.fn.serviceCall('PUT', '', 'restapi/twitterscrapper/addsource/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.addTwitterDestination = function(id, accountid) {
	$.fn.serviceCall('PUT', '', 'restapi/twitterscrapper/adddestination/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.addTwitterMiddle = function(id, accountid) {
	$.fn.serviceCall('PUT', '', 'restapi/twitterscrapper/addmiddle/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.addTwitterOrder = function(id, accountid) {
	$.fn.serviceCall('PUT', '', 'restapi/twitterscrapper/addorder/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.deleteTwitterSource = function(id, accountid) {
	$.fn.serviceCall('DELETE', '', 'restapi/twitterscrapper/deletesource/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.deleteTwitterDestination = function(id, accountid) {
	$.fn.serviceCall('DELETE', '', 'restapi/twitterscrapper/deletedestination/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.deleteTwitterMiddle = function(id, accountid) {
	$.fn.serviceCall('DELETE', '', 'restapi/twitterscrapper/deletemiddle/' + id + '/' + accountid, 45000, function(data) {
	});
}

$.fn.deleteTwitterOrder = function(id, accountid) {
	$.fn.serviceCall('DELETE', '', 'restapi/twitterscrapper/deleteorder/' + id + '/' + accountid, 45000, function(data) {
	});
}

// Get all Scrappers for user
$.fn.getTwitterScrapperList = function() {
	// Clear out the data
	$('#scrappername').val('');
	...
	$.fn.serviceCall('GET', '', 'restapi/twitterscrapper/userid/' + uId, 45000, function(data) {
    		var liString = '';
        for (var i=0; (typeof data != 'undefined') && (data != null) && (i < data.length); ++i)
        {
	        	var odd = i % 2;
        ...
        $('#scrapper-list a').on('click', function(e) {
	        	...
	        	$.fn.getTwitterScrapperListings(idSelected);
        });
...
$.fn.addTwitterScrapper = function() {
	if ($('#scrappername').val() === '' ||
		$('#spreadlineadjustment').val() === '' || 
		$('#spreadmaxamount').val() === '' ||  
		$("#totallineadjustment").val() === '' ||
		...
		$('#onoff').val() === '')
	{
		...
		log(scrapperObject);
		$.fn.serviceCall('POST', scrapperObject, 'restapi/twitterscrapper/add', 45000, function(data) {
            $.fn.getTwitterScrapperList();
		});
	}
}

// Function to update an account
$.fn.updateTwitterScrapper = function() {
	if ($('#scrappername').val() === '' ||
		$('#spreadlineadjustment').val() === '' || 
		$('#spreadmaxamount').val() === '' ||  
	    $("#totallineadjustment").val() === '' ||
		...
		log(scrapperObject);
		$.fn.serviceCall('POST', scrapperObject, 'restapi/twitterscrapper/update', 45000, function(data) {
            $.fn.getTwitterScrapperList();
		});
	}
}
...
$.fn.deleteTwitterScrapper = function() {
	var id = $("[id='" + $('#scrappernameid').val() + "_id']").val();

	$.fn.serviceCall('DELETE', '', 'restapi/twitterscrapper/delete/' + id, 45000, function(data) {
		// Call the service to get all accounts
		$.fn.getTwitterScrapperList();
...
$.fn.loadTwitterScrapperData = function() {
	selectedList = [];
