	...
	$.fn.getTransactionInfo = function(url) {
		// Only refresh for two minutes
		if (retryCount++ < 240) {
			$.fn.serviceCallNoSpinner('GET', '', url, 45000, function(data) {
	...
	$.fn.recordSpreadEvent = function(eventObject) {
		log(eventObject);
		$.fn.serviceCall('POST', eventObject, 'restapi/recordevent/spread', 45000, function(data) {
			log(data);
	...
	$.fn.recordTotalEvent = function(eventObject) {
		log(eventObject);
		$.fn.serviceCall('POST', eventObject, 'restapi/recordevent/total', 45000, function(data) {
			log(data);
	...
	$.fn.recordMlEvent = function(eventObject) {
		log(eventObject);
		$.fn.serviceCall('POST', eventObject, 'restapi/recordevent/ml', 45000, function(data) {
			log(data);
...
//       	$('#error-message').addClass('error_message_on');
	...
	$.fn.sendRecord = function() {
	   ...
	   if (submitType === 'spread') {
	    	   ...
	    	   var spreadTransaction = '{' +
	    	   		'"spreadRecordEvent":' + spreadEventObject + ',' + 
	    	   		'"accounts":[' + accountid + '],' +
	    	   		'"groups":[' + groupid + ']}';
		   $(this).recordSpreadEvent(spreadTransaction);
	    	   ...
	    	   var totalTransaction = '{' +
   	   		'"totalRecordEvent":' + totalEventObject + ',' + 
   	   		'"accounts":[' + accountid + '],' +
   	   		'"groups":[' + groupid + ']}';
		   $(this).recordTotalEvent(totalTransaction);
	    	   ...
	    	   var mlTransaction = '{' +
	   	   		'"mlRecordEvent":' + mlEventObject + ',' + 
	   	   		'"accounts":[' + accountid + '],' +
	   	   		'"groups":[' + groupid + ']}';
		   $(this).recordMlEvent(mlTransaction);            		   
	   }
	   $(this).dialog("close");
	}
	...
	$.fn.setupTransaction = function() {
		...
		$.fn.serviceCall('GET', '', 'restapi/event?eventid=' + eventid, 45000, function(data) {
			log(data);
			if (typeof data != 'undefined' && typeof data != null) {
