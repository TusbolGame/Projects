package com.wooanalytics.dao.ncaab;
...
import com.wooanalytics.dao.sites.kenpom.KenPomSite;
import com.wooanalytics.dao.sites.masseyratings.MasseyRatingsData;
import com.wooanalytics.dao.sites.teamrankings.TeamRankingsData;
import com.wooanalytics.dao.sites.usatoday.SagarinData;
 ...
 *
 */
public class WoONcaaBasketballReport extends NcaaBasketballReport {
	private final static Logger LOGGER = Logger.getLogger(WoONcaaBasketballReport.class);

	/**
	 * 
	 */
	public WoONcaaBasketballReport() {
		super();
	}
	...
	public static void main(String[] args) {
		try {
			...
			kps.getHttpClientWrapper().setupHttpClient("None");
			...
			for (VegasInsiderGame vig : games) {
				...
				String roadTeam = vig.getAwayteamdata().getTeamname().toLowerCase();
				roadTeam = roadTeam.replace("'", "");
				String homeTeam = vig.getHometeamdata().getTeamname().toLowerCase();
				homeTeam = homeTeam.replace("'", "");

				for (KenPomData kp : lkpd) {
					LOGGER.error("kpd: " + kp);
					String kproadTeam = kp.getRoadTeam();
					String kphomeTeam = kp.getHomeTeam();

					if (kproadTeam != null && kphomeTeam != null) {
						kproadTeam = kproadTeam.replace("'", "").toLowerCase();
						kphomeTeam = kphomeTeam.replace("'", "").toLowerCase();

						if (kphomeTeam.equals(homeTeam.toLowerCase()) && 
							kproadTeam.equals(roadTeam.toLowerCase())) {
							LOGGER.error("FOUND");
						}
					}
				}
	 ...
	 * @return
	 */
	public ByteArrayOutputStream createNcaaReport() {
	...
	public HSSFWorkbook generateReportWorkbook() {
		...
		int count = 0;
		for (VegasInsiderGame vigs : vig) {
			if (vigs.getAwayteamdata().getTeamname() != null && vigs.getAwayteamdata().getTeamname().length() > 0 && 
				vigs.getHometeamdata().getTeamname() != null && vigs.getHometeamdata().getTeamname().length() > 0) {
				// Create the worksheet
				String roadTeam = vigs.getAwayteamdata().getTeamname().toLowerCase();
				roadTeam = roadTeam.replace("'", "");
				String homeTeam = vigs.getHometeamdata().getTeamname().toLowerCase();
				homeTeam = homeTeam.replace("'", "");
				...
				if (kpd == null) {
					...
					setupGameInfo(cal, count++, roadTeam, homeTeam, siteType, sheetName, vigs);
				}
			}
		}
	 ...
	 * @param vigs
	 */
	public void setupGameInfo(HSSFSheet cal, int count, String roadTeam, String homeTeam, int siteType, String sheetName, VegasInsiderGame vigs) {
		HSSFRow rowhead = cal.createRow(count+2);
		count = count + 3;
		final Calendar gametime = Calendar.getInstance(TimeZone.getTimeZone("America/Chicago"));

		if (vigs.getDate() != null) {
			gametime.setTime(vigs.getDate());
		} else {
			gametime.setTime(new Date());
		}

		int month = gametime.get(Calendar.MONTH) + 1;
		int day = gametime.get(Calendar.DAY_OF_MONTH);
		int year = gametime.get(Calendar.YEAR);
		int hour = gametime.get(Calendar.HOUR);
		...
		rowhead.createCell(0).setCellValue(monthString + "-" +
				dayString + "-" +
				year + " " + 
				hourString + ":" + 
				minuteString);
		...
		if (masseyComposite != null && masseyComposite.size() > 0) {
			...
			while (itr.hasNext()) {
				...
				if (key != null && roadTeam != null && homeTeam != null) {
					key = key.replace("'", "");
					if (key.toLowerCase().equals(roadTeam.toLowerCase())) {
						roadNumber = value;
					} else if (key.toLowerCase().equals(homeTeam.toLowerCase())) {
						homeNumber = value;
					}
				}
			}
		}
		rowhead.createCell(1).setCellValue(vigs.getAwayteamdata().getRotationid());
		rowhead.createCell(2).setCellValue(vigs.getHometeamdata().getRotationid());
		rowhead.createCell(3).setCellValue("(" + roadNumber + ") " + roadTeam.toUpperCase());
		rowhead.createCell(4).setCellValue("(" + homeNumber + ") " + homeTeam.toUpperCase());
		
		if (roadTeam.toLowerCase().equals(vigs.getLinefavorite().toLowerCase())) {
			final String line = Float.toString(vigs.getLine()).replace("-", "");
			rowhead.createCell(5).setCellValue(Float.parseFloat(line));
		} else {
			rowhead.createCell(5).setCellValue(vigs.getLine());
		}
		...
		HSSFCell columno = rowhead.createCell(14);
		HSSFCell columnp = rowhead.createCell(15);
		HSSFCell columnq = rowhead.createCell(16);

		columng.setCellType(CellType.FORMULA);
		columnh.setCellType(CellType.FORMULA);
		columni.setCellType(CellType.FORMULA);
		columnj.setCellType(CellType.FORMULA);
		columnk.setCellType(CellType.NUMERIC);
		columnl.setCellType(CellType.NUMERIC);
		columnm.setCellType(CellType.NUMERIC);
		columnn.setCellType(CellType.FORMULA);
		columno.setCellType(CellType.FORMULA);
		columnp.setCellType(CellType.FORMULA);
		columnq.setCellType(CellType.FORMULA);

		columng.setCellFormula("'" + sheetName + "'" + "!D59");
		columnh.setCellFormula("'" + sheetName + "'" + "!D29");
		columni.setCellFormula("'" + sheetName + "'" + "!D89");
		columnj.setCellFormula("'" + sheetName + "'" + "!D119");
		...
		for (SagarinData sd : sagarinRatings) {
			...
			if (sdTeam != null) {
				sdTeam = sdTeam.replace("'", "").toLowerCase();
	
				if (sdTeam.equals(roadTeam.toLowerCase())) {
					teamOne = sd.getValue();
				} else if (sdTeam.equals(homeTeam.toLowerCase())) {
					teamTwo = sd.getValue();
				}
			}
		}
		...
		columnk.setCellValue(teamOne - teamTwo - siteType);
		...
		for (KenPomData kpd : lkpd) {
			...
			if (kpdRoadTeam != null && kpdHomeTeam != null) {
				kpdRoadTeam = kpdRoadTeam.replace("'", "").toLowerCase();
				kpdHomeTeam = kpdHomeTeam.replace("'", "").toLowerCase();
				
				if (kpdRoadTeam.equals(roadTeam.toLowerCase()) &&
					kpdHomeTeam.equals(homeTeam.toLowerCase())) {
					kpdSpread = kpd.getSpread();
				}
			}
		}
		columnl.setCellValue(kpdSpread);

		float masseySpread = 0;
		for (MasseyRatingsData ms : masseySpreads) {
			String kpdRoadTeam = ms.getOpponentTeam();
			String kpdHomeTeam = ms.getTeam();

			if (kpdRoadTeam != null && kpdHomeTeam != null) {
				if (kpdRoadTeam.toLowerCase().equals(roadTeam.toLowerCase()) &&
					kpdHomeTeam.toLowerCase().equals(homeTeam.toLowerCase()) &&
					ms.getLinefavorite() != null) {
					if (roadTeam.toLowerCase().equals(ms.getLinefavorite().toLowerCase())) {
						final String line = Float.toString(ms.getSpread()).replace("-", "");
						masseySpread = Float.parseFloat(line);
					} else {
						masseySpread = ms.getSpread();
					}
				}
			}
		}
		columnm.setCellValue(masseySpread); // Massey
		columnn.setCellFormula("AVERAGE(K" + count + ":M" + count + ")");
		columno.setCellFormula("G" + count + "*0.2+H" + count + "*0.2+I" + count + "*0.05+J" + count + "*0.1+K" + count + "*0.25+L" + count + "*0.2");
		columnp.setCellFormula("(ABS(O" + count + "-N" + count + "))");
	}
	...
	public void setupCalHeader(HSSFSheet cal) {
		...
		rowhead.createCell(0).setCellValue("Game Date");
		rowhead.createCell(1).setCellValue("Road Rotation ID");
		rowhead.createCell(2).setCellValue("Home Rotation ID");
		rowhead.createCell(3).setCellValue("Road Team");
		rowhead.createCell(4).setCellValue("Home Team");
		rowhead.createCell(5).setCellValue("Home Spread");
		rowhead.createCell(6).setCellValue("2017");
		rowhead.createCell(7).setCellValue("2018");
		rowhead.createCell(8).setCellValue("Last 3");
		rowhead.createCell(9).setCellValue("GK");
		rowhead.createCell(10).setCellValue("Sagarian");
		rowhead.createCell(11).setCellValue("Ken Pom");
		rowhead.createCell(12).setCellValue("Massey");
		rowhead.createCell(13).setCellValue("Spread");
		rowhead.createCell(14).setCellValue("BP computed");
		rowhead.createCell(15).setCellValue("Difference");
		rowhead.createCell(16).setCellValue("Massey Composite");
	}
	...
	private void setupReportData() {
		...
		masseySpreads = REPORT_CACHE.getMasseySpreads();
		vig = REPORT_CACHE.getVig();
