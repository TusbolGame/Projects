	...
	$.fn.addGroup = function() {
		if ($('#groupname').val() === '') 
		{
			...
			log(groupObject);
			$.fn.serviceCall('POST', groupObject, 'restapi/group/id/' + uId, 15000, function(data) {
	            $.fn.getGroupList();
			});
		}
	}
	...
	$.fn.updateGroup = function() {
		if ($('#groupname').val() === '') 
		{
			...
			log(groupObject);
			$.fn.serviceCall('POST', groupObject, 'restapi/group/update', 15000, function(data) {
	            $.fn.getGroupList();
			});
		}
	}
	...
	$.fn.deleteGroup = function() {
		var id = $('#' + $('#groupnameid').val() + '_id').val();
		$.fn.serviceCall('DELETE', '', 'restapi/group/delete/' + id + '?userid=' + uId, 15000, function(data) {
