...
import com.ticketadvantage.services.batch.processevent.WebGlobalSiteBatch;
import com.ticketadvantage.services.batch.processevent.WebUserSiteBatch;
...
@WebListener
@Service
public class WebScrapperEventBatch implements Runnable, ServletContextListener {
	private static final Logger LOGGER = Logger.getLogger(WebScrapperEventBatch.class);
	 ...
	 * 
	 */
	public WebScrapperEventBatch() {
		super();
		LOGGER.info("Entering WebScrapperEventBatch()");
		LOGGER.info("Exiting WebScrapperEventBatch()");
	}
	
	public class UserContainer {
		String id;
	    WebUserSiteBatch userSiteBatch;
	...
	public class GlobalContainer {
		String id;
	    WebGlobalSiteBatch globalSiteBatch;
	...
	public static void main(String[] args) {
		WebScrapperEventBatch scrapperEventBatch = new WebScrapperEventBatch();
			...
			try {
				...
				for (WebUserScrapper us : userList) {
					...
					if (us.getUserScrappers() != null && !us.getUserScrappers().isEmpty()) {
						...
						if (uc.isEmpty()) {
							...
							final WebUserSiteBatch usb = new WebUserSiteBatch(us);
				...
				for (WebGlobalScrapper gs : globalList) {
					...
					if (gs.getUserScrappers() != null && !gs.getUserScrappers().isEmpty()) {
						if (gc.isEmpty()) {
							final WebGlobalSiteBatch gsb = new WebGlobalSiteBatch(gs);
