 ...
 */
@Service
public class PinnyWebGlobalSiteBatch extends WebBaseSiteBatch implements Runnable {
	private static final Logger LOGGER = Logger.getLogger(PinnyWebGlobalSiteBatch.class);
	 ...
	 * 
	 */
	public PinnyWebGlobalSiteBatch() {
		super();
	 ...
	 * 
	 */
	public PinnyWebGlobalSiteBatch(GlobalScrapper globalScrapper) {
		super();
		LOGGER.info("Entering WebGlobalSiteBatch()");
		LOGGER.info("Exiting WebGlobalSiteBatch()");
	...
	public void run() {
		...
		try {
			setupSiteAndLogin();
			int pullingInterval = Integer.parseInt(globalScrapper.getUserScrappers().get(0).getPullinginterval()) * 1000;
	
			while (!shutdown) {
				try {
					checkSite("global", this.globalScrapper.getUserScrappers(), this.globalScrapper.getUserid());
			...
			for (PendingEvent sitePendingEvent : sitePendingEvents) {
				...
				if (scrapper.getUserid() != null && scrapper.getUserid().longValue() == 1) {
					...
					for (int x = 0; x < XPIKE1_WOO_PLAYERS.length; x++) {
						if (sitePendingEvent.getCustomerid().equals(XPIKE1_WOO_PLAYERS[x][0])) {
							Double win = Double.valueOf(sitePendingEvent.getWin().replace("$", "").replaceAll(",", ""));
							Double wooWin = Double.valueOf(XPIKE1_WOO_PLAYERS[x][1]);
							if (win >= wooWin) {
								found = true;
							}
						}			
					...
					for (int x = 0; x < XPIKE4_WOO_PLAYERS.length; x++) {
						if (sitePendingEvent.getCustomerid().equals(XPIKE4_WOO_PLAYERS[x][0])) {
							Double win = Double.valueOf(sitePendingEvent.getWin().replace("$", "").replaceAll(",", ""));
							Double wooWin = Double.valueOf(XPIKE4_WOO_PLAYERS[x][1]);
							if (win >= wooWin) {
								found = true;
							}
						}			
					...
					for (int x = 0; x < XPIKE5_WOO_PLAYERS.length; x++) {
						if (sitePendingEvent.getCustomerid().equals(XPIKE5_WOO_PLAYERS[x][0])) {
							Double win = Double.valueOf(sitePendingEvent.getWin().replace("$", "").replaceAll(",", ""));
							Double wooWin = Double.valueOf(XPIKE5_WOO_PLAYERS[x][1]);
							if (win >= wooWin) {
								found = true;
							}
						}			
					...
					for (int x = 0; x < XPIKE6_WOO_PLAYERS.length; x++) {
						if (sitePendingEvent.getCustomerid().equals(XPIKE6_WOO_PLAYERS[x][0])) {
							Double win = Double.valueOf(sitePendingEvent.getWin().replace("$", "").replaceAll(",", ""));
							Double wooWin = Double.valueOf(XPIKE6_WOO_PLAYERS[x][1]);
							if (win >= wooWin) {
								found = true;
							}
						}			
				...
				if (found) {
					...
					if (isSportAvailable(sitePendingEvent.getGamesport(), sitePendingEvent.getGametype(), sitePendingEvent.getEventtype(), sitePendingEvent.getLinetype(), scrapper)) {
						...
						LOGGER.debug("foundPending: " + foundPending);
	...
	private void setupSiteAndLogin() throws BatchException {
		...
		if (globalScrapper != null && globalScrapper.getUserScrappers() != null && globalScrapper.getUserScrappers().size() > 0) {
			...
			final List<BaseScrapper> scrappers = globalScrapper.getUserScrappers();
