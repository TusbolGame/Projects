 ...
 *
 */
public class WoOzNcaaBasketballReport extends NcaaBasketballReport {
	private final static Logger LOGGER = Logger.getLogger(WoOzNcaaBasketballReport.class);

	/**
	 * 
	 */
	public WoOzNcaaBasketballReport() {
		super();
	}

	// 3pt fg made/attempted 20%/10%
	// ft made/attempted 15%/5%
	// Off Rebounds 10%
	// Def Rebounds 8%
	// Opp turnovers 8%
	// Steals 8%
	...
	public HSSFWorkbook generateReportWorkbook() {
		...
		opponentfgMadePerGame = REPORT_CACHE.getOpponentfgMadePerGame();
		opponentfgAttemptedPerGame = REPORT_CACHE.getOpponentfgAttemptedPerGame();
		opponentthreeFgMadePerGame = REPORT_CACHE.getOpponentthreeFgMadePerGame();
		opponentthreeFgAttemptedPerGame = REPORT_CACHE.getOpponentthreeFgAttemptedPerGame();
		opponentftMadePerGame = REPORT_CACHE.getOpponentftMadePerGame();
		opponentftAttemptedPerGame = REPORT_CACHE.getOpponentftAttemptedPerGame();
		...
		for (KenPomData kpd : lkpd) {
			...
			if (kpd.getRoadTeam() != null && kpd.getRoadTeam().length() > 0 && 
				...
				int siteType = -4;
				int siteLocationType = 1;
				for (KenPomData kp : lkpd) {
					if (kp.getHomeTeam() != null && kp.getHomeTeam().equals(homeTeam)) {
						siteType = kp.getSiteLocation();
						siteLocationType = kp.getSiteLocationType();
					}
				}

				createNcaab2018ModelForTeam(percentStyle, sheet, kpd, roadTeam, homeTeam, siteType, siteLocationType);
				...
				setupGameInfo(cal, count++, roadTeam, homeTeam, siteType, siteLocationType);
			}
		}
	 ...
	 * @param homeTeam
	 * @param siteType
	 * @param siteLocationType
	 */
	public void setupGameInfo(HSSFSheet cal, int count, String roadTeam, String homeTeam, int siteType, int siteLocationType) {
		HSSFRow rowhead = cal.createRow(count+2);
		count = count + 3;
		rowhead.createCell(0).setCellValue("");
		rowhead.createCell(1).setCellValue(roadTeam);
		rowhead.createCell(2).setCellValue(homeTeam);
		...
		columnd.setCellFormula("'" + sheetName + "'" + "!D63");
		columne.setCellFormula("'" + sheetName + "'" + "!D33");
		columnf.setCellFormula("'" + sheetName + "'" + "!D93");
		columng.setCellFormula("'" + sheetName + "'" + "!D123");

		float teamOne = 0;
		float teamTwo = 0;
		for (SagarinData sd : sagarinRatings) {
			if (sd.getTeam().equals(roadTeam)) {
				teamOne = sd.getValue();
			} else if (sd.getTeam().equals(homeTeam)) {
				teamTwo = sd.getValue();
			}
		}
		...
		int kpdSpread = 0;
		for (KenPomData kpd : lkpd) {
			if (kpd.getRoadTeam() != null && kpd.getRoadTeam().equals(roadTeam)) {
	 ...
	 * @param siteLocationType
	 */
	public void createNcaab2018ModelForTeam(HSSFCellStyle percentStyle, HSSFSheet sheet, KenPomData kpd, String teamOne, String teamTwo, int siteType, int siteLocationType) {
		int count = 0;
		...
		rowhead.createCell(0).setCellValue("FG Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : fgMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
					efp1 = tpp.getField7();
				} else {
					efp1 = tpp.getField3();
				}
				...
				String efp2 = null;
				if (siteLocationType == 1 || siteLocationType == 3) {
					efp2 = tpp.getField6();
				} else {
					efp2 = tpp.getField3();
				}
		...
		rowhead.createCell(0).setCellValue("FG Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.fgAttemptedPerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		// Row6 - Opponent FG Made
		rowhead = sheet.createRow(count + 5);
		rowhead.createCell(0).setCellValue("Opponent FG Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentfgMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F6+F7) * 0.5");
		columnf.setCellFormula("C6-D6");

		// Row7 - Opponent FG Attempted
		rowhead = sheet.createRow(count + 6);
		rowhead.createCell(0).setCellValue("Opponent FG Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentfgMadePerGame) {
		...
		columnf.setCellFormula("-(C7-D7)");

		// Row8 - 3pt FG Made
		rowhead = sheet.createRow(count + 7);
		rowhead.createCell(0).setCellValue("3pt FG Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.threeFgMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F8+F9) * 0.5");
		columnf.setCellFormula("C8-D8");

		// Row9 - 3pt FG Attempted
		rowhead = sheet.createRow(count + 8);
		rowhead.createCell(0).setCellValue("3pt FG Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.threeFgAttemptedPerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("-(C9-D9)");

		// Row10 - Opponent 3pt FG Made
		rowhead = sheet.createRow(count + 9);
		rowhead.createCell(0).setCellValue("Opponent 3pt FG Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentthreeFgMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F10+F11) * 0.5");
		columnf.setCellFormula("C10-D10");

		// Row11 - Opponent 3pt FG Attempted
		rowhead = sheet.createRow(count + 10);
		rowhead.createCell(0).setCellValue("Opponent 3pt FG Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentthreeFgAttemptedPerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("-(C11-D11)");

		// Row12 - FT Made
		rowhead = sheet.createRow(count + 11);
		rowhead.createCell(0).setCellValue("FT Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.ftMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		rowhead.createCell(0).setCellValue("FT Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.ftAttemptedPerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		// Row14 - Opponent FT Made
		rowhead = sheet.createRow(count + 13);
		rowhead.createCell(0).setCellValue("Opponent FT Made");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentftMadePerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F14+F15) * 0.5");
		columnf.setCellFormula("C14-D14");

		// Row15 - Opponent FT Attempted
		rowhead = sheet.createRow(count + 14);
		rowhead.createCell(0).setCellValue("Opponent FT Attempted");
		rowhead.createCell(1).setCellValue("");
		columnc = rowhead.createCell(2);
		columnd = rowhead.createCell(3);
		for (TeamRankingsData tpp : this.opponentftAttemptedPerGame) {
			if (tpp.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = null;
				if (siteLocationType == 2 || siteLocationType == 4) {
		...
		// Row17 - Opponents Steals Per Game
		rowhead = sheet.createRow(count + 12);
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F18+F19) * 0.4");
		columnf.setCellFormula("C18-D18");

		// Row19 - Opponents Blocks Per Game
		...
		// Row20 - Offensive Rebounds Per Game
		...
		// Row21 - Opponent Offensive Rebounds Per Game
		...
		// Row22 - Defensive Rebounds Per Game
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F22+F23) * 0.25");
		columnf.setCellFormula("C22-D22");

		// Row23 - Opponent Defensive Rebounds Per Game
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("-(C23-D23)");

		// Row24 - Assists Per Game
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F24+F25) * 0.5");
		columnf.setCellFormula("C24-D24");

		// Row25 - Opponents Assists Per Game
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F26+F27) * 0.5");
		columnf.setCellFormula("-(C26-D26)");

		// Row27 - Opponents Turnovers Per Game
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("(C27-D27) * 0.6");

		// Row28 - Personal Fouls Per Game
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("(F28+F29) * 0.4");
		columnf.setCellFormula("-(C28-D28)");

		// Row29 - Opponent Personal Fouls Per Game
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("C29-D29");

		// Row30 - Extra Chances Per Game
		rowhead = sheet.createRow(count + 29);
		...
		columne.setCellType(CellType.FORMULA);
		columnf.setCellType(CellType.FORMULA);
		columne.setCellFormula("F30 * 0.05");
		columnf.setCellFormula("C30-D30");

		// Row31 - Schedule Strength By Other
		rowhead = sheet.createRow(count + 30);
		...
		columnd = rowhead.createCell(3);
		columnd.setCellType(CellType.FORMULA);
		columnd.setCellFormula("SUM(E2:E31) - D31 + C31 - " + siteType);
	}
	...
	public void createNcaab2017ModelForTeam(HSSFCellStyle percentStyle, HSSFSheet sheet, KenPomData kpd, String teamOne, String teamTwo, int siteType) {
		...
		for (TeamRankingsData ppg : possessionsPerGame) {
			if (ppg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = ppg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (ppg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = ppg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData oe : offensiveEfficiency) {
			if (oe.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = oe.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (oe.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = oe.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData spg : stealsPerGame) {
			if (spg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = spg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (spg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = spg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData bpg : blocksPerGame) {
			if (bpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = bpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (bpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = bpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData obpg : opponentBlocksPerGame) {
			if (obpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = obpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (obpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = obpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData orpg : offensiveReboundsPerGame) {
			if (orpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = orpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (orpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = orpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData oorpg : oponentOffensiveReboundsPerGame) {
			if (oorpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = oorpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (oorpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = oorpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData drpg : defensiveReboundsPerGame) {
			if (drpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = drpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (drpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = drpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData odrpg : opponentDefensiveReboundsPerGame) {
			if (odrpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = odrpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (odrpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = odrpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData apg : assistsPerGame) {
			if (apg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = apg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (apg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = apg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData oapg : opponentAssistsPerGame) {
			if (oapg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = oapg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (oapg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = oapg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData tpg : turnoversPerGame) {
			if (tpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = tpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (tpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = tpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData otpg : opponentTurnoversPerGame) {
			if (otpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = otpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (otpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = otpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		columnf.setCellType(CellType.FORMULA);
		columne.setCellValue("");
		columnf.setCellFormula("(C53-D53) * 0.6");
		...
		for (TeamRankingsData pfpg : personalFoulsPerGame) {
			if (pfpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = pfpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (pfpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = pfpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData opfpg : oppoenentPersonalFoulsPerGame) {
			if (opfpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = opfpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (opfpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = opfpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
		...
		for (TeamRankingsData ecpg : extraChancesPerGame) {
			if (ecpg.getField2().equals(kpd.getRoadTeam())) {
				columnc.setCellType(CellType.NUMERIC);
				String efp1 = ecpg.getField8();
				efpint1 = Float.parseFloat(efp1);
				columnc.setCellValue(efpint1);
			} else if (ecpg.getField2().equals(kpd.getHomeTeam())) {
				columnd.setCellType(CellType.NUMERIC);
				String efp2 = ecpg.getField8();
				efpint2 = Float.parseFloat(efp2);
				columnd.setCellValue(efpint2);
			}
		}
