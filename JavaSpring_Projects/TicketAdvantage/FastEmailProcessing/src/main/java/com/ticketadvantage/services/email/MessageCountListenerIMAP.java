...
public class MessageCountListenerIMAP implements MessageCountListener {
	...
	/**
	 * 
	 * @param fastEmailEventBatch
	 */
	public MessageCountListenerIMAP(DrHEmailEventBatch drHEmailEventBatch) {
		super();
		...
		if (messages != null && messages.length > 0) {
			...
			for (Message message : messages) {
				...
				try {
					...
					LOGGER.debug("fromaddresses: " + fromaddresses);
					...
					LOGGER.debug("bccaddresses: " + bccaddresses);
					emailContainer.setBccemail(bccaddresses);

					LOGGER.error("EmailContainer: " + emailContainer);
	
					// Process this email
					if (drHEmailEventBatch != null) {
						drHEmailEventBatch.processEmail(emailContainer);
					}
