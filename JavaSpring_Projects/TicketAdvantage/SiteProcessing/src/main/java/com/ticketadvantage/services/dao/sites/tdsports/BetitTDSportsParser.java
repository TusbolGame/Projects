 ...
 *
 */
public class BetitTDSportsParser extends TDSportsParser {
	private static final Logger LOGGER = Logger.getLogger(BetitTDSportsParser.class);
	...
	public Map<String, String> parseMenu(String xhtml, String[] type, String[] sport) throws BatchException {
		...
		map = findMenu(doc.select("#mesax ul li h4 div"), map, type, sport, "label a", "label input");

		// Remove the input fields that shouldn't be sent
		map = removeInputFields(map);
		map.remove("ctl00$WagerContent$btn_Continuemod11");

		// Continue button
		map.put("ctl00$WagerContent$btn_Continuemod1", "Continue");
	...
	public <TDSportsEventPackage> List<TDSportsEventPackage> parseGames(String xhtml, String type, Map<String, String> inputFields) throws BatchException {
		...
		Elements elements = doc.select(".Wgrtbl2 tbody tr");
