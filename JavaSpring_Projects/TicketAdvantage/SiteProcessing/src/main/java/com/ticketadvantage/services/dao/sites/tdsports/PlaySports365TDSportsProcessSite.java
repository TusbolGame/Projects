 ...
 *
 */
public class PlaySports365TDSportsProcessSite extends TDSportsProcessSite {
	private static final Logger LOGGER = Logger.getLogger(PlaySports365TDSportsProcessSite.class);
	protected PlaySports365TDSportsParser TDP = new PlaySports365TDSportsParser();

	/**
	 * 
	 */
	public PlaySports365TDSportsProcessSite(String host, String username, String password, boolean isMobile, boolean showRequestResponse) {
		super("Playsports365TDSports", host, username, password, isMobile, showRequestResponse);
		LOGGER.info("Entering PlaySports365TDSportsProcessSite()");
		...
		LOGGER.info("Exiting PlaySports365TDSportsProcessSite()");
	}
	...
	public static void main(String[] args) {
		try {
			final String[][] TDSITES = new String [][] { 
				{ "http://playsports365.com/", "PR990818", "NYC", "1000", "1000", "1000", "New York City", "ET"}
			};

			// Loop through the sites
			for (int i = 0; i < TDSITES.length; i++) {
				final PlaySports365TDSportsProcessSite processSite = new PlaySports365TDSportsProcessSite(TDSITES[i][0], TDSITES[i][1],
						TDSITES[i][2], false, true);
				final PlaySports365TDSportsProcessSite processSite2 = new PlaySports365TDSportsProcessSite(TDSITES[i][0], TDSITES[i][1],
						TDSITES[i][2], false, true);
			    ...
			    processSite.loginToSite("PR990818", "NYC");

			    Set<PendingEvent> pendingEvents = processSite.getPendingBets("PR990818", "NYC", null);
