 ...
 *
 */
public class GsbettingProcessSite extends HeritageSportsProcessSite {
	private static final Logger LOGGER = Logger.getLogger(GsbettingProcessSite.class);
	...
	private final GsbettingParser GSP = new GsbettingParser();
	 ...
	 */
	public GsbettingProcessSite(String host, String username, String password, boolean isMobile, boolean showRequestResponse) {
		super("Gsbetting", host, username, password, isMobile, showRequestResponse);
		LOGGER.info("Entering GsbettingProcessSite()");

		// Setup the parser
		this.siteParser = GSP;
		GSP.setTimezone(this.timezone);
		...
		LOGGER.info("Exiting GsbettingProcessSite()");
	}
	...
	public static void main(String[] args) {
		try {
			final String[][] HSSITES = new String [][] {
				{ "http://www.gsbetting.net", "A910", "6688", "500", "500", "500", "Los Angeles", "PT" }
			};

			// Loop through the sites
			for (int i = 0; i < HSSITES.length; i++) {
				final GsbettingProcessSite processSite = new GsbettingProcessSite(HSSITES[i][0], HSSITES[i][1],
			    ...
			    Map<String, String> map = processSite.getMenuFromSite("ncaabsecond", xhtml);
			    LOGGER.debug("map: " + map);

				xhtml = processSite.selectSport("ncaabsecond");
				LOGGER.debug("XHTML: " + xhtml);

				final Iterator<EventPackage> ep = processSite.parseGamesOnSite("ncaabsecond", xhtml);
	...
	/*
	 * (non-Javadoc)
	 * @see com.ticketadvantage.services.dao.sites.heritagesports.HeritageSportsProcessSite#getPendingBets(java.lang.String, java.lang.String, java.lang.Object)
	 */
	@Override
	public Set<PendingEvent> getPendingBets(String accountName, String accountId, Object anythingObject) throws BatchException {
		...
		if (xhtml != null && xhtml.contains("view all wagers graded within the last 14 days.")) {
			pendingWagers = GSP.parsePendingBets(xhtml, accountName, accountId);
		} else {
	...
	protected void parseEventSelection(String xhtml, SiteTransaction siteTransaction, EventPackage eventPackage, BaseRecordEvent event, AccountEvent ae) throws BatchException {
		...
		MAP_DATA = GSP.parseEventSelection(xhtml, null, null);
		LOGGER.debug("MAP_DATA: " + MAP_DATA);

		// Parse the wager types
		Map<String, String> wagers = GSP.parseWagerType(xhtml);
		...
		if (wagers.containsKey("wageramount")) {
			...
			if (rerunCount++ == 0) {
				...
				MAP_DATA = GSP.parseWagerType(xhtml);
	...
	protected String parseTicketTransaction(String xhtml) throws BatchException {
		...
		final String ticketNumber = GSP.parseTicketNumber(xhtml);
