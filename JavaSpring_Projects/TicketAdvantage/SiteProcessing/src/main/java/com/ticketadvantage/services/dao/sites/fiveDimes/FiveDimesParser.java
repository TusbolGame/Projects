 ...
 *
 */
public class FiveDimesParser extends SiteParser {
	private final static Logger LOGGER = Logger.getLogger(FiveDimesParser.class);
	...
	public static void main(String[] args) {
		try {
			FiveDimesParser FiveDimesParser = new FiveDimesParser();
			/*
			 * String html =
...
//		<span class=rotnumb > -185 for Game <br>S. Strasburg - R must Start&nbsp;&nbsp;A. Nola - R must Start </span>
	...
	@SuppressWarnings("unchecked")
	@Override
	protected List<FiveDimesEventPackage> getGameData(Elements elements) throws BatchException {
		LOGGER.info("Entering getGameData()");
		final List<FiveDimesEventPackage> events = new ArrayList<FiveDimesEventPackage>();

		if (elements != null) {
			FiveDimesTeamPackage team1 = null;
			FiveDimesTeamPackage team2 = null;
			FiveDimesEventPackage eventPackage = null;
			// int count=0;
			...
			for (int x = 0; x < elements.size(); x++) {
				...
				if (element != null) {
					...
					if (tables1 != null && tables1.size() > 0 && tables2 != null && tables2.size() > 0) {
						...
						team1 = new FiveDimesTeamPackage();
						team2 = new FiveDimesTeamPackage();
						eventPackage = new FiveDimesEventPackage();
...
/*								final Elements row1_gameTeam = row1.select("tr").eq(0);
	 ...
	 * @throws BatchException
	 */
	protected FiveDimesTeamPackage getGameTeam(Elements elements, FiveDimesTeamPackage team) throws BatchException {
	 ...
	 * @throws BatchException
	 */
	private FiveDimesTeamPackage getTeam(Element td, FiveDimesTeamPackage team) throws BatchException {
	 ...
	 * @throws BatchException
	 */
	private FiveDimesTeamPackage getSpread(Element td, FiveDimesTeamPackage team) throws BatchException {
		...
		if (options != null && options.size() > 0) {
			team = (FiveDimesTeamPackage) parseSpreadSelectOption(options, team, " ", null);
		} else {
			// -2½ +200; Now parse the data
			String data = getHtmlFromLastIndex(td, ">");
			team = (FiveDimesTeamPackage) parseSpreadData(reformatValues(data), 0, " ", null, team);
		}
	 ...
	 * @return
	 */
	private FiveDimesTeamPackage getMoneyLine(Element td, FiveDimesTeamPackage team) {
		...
		team = (FiveDimesTeamPackage) parseMlData(data, 0, team);
	 ...
	 * @throws BatchException
	 */
	private FiveDimesTeamPackage getTotal(Element td, FiveDimesTeamPackage team) throws BatchException {
		...
		if(selectStr.length()>0) {
		  ...
		  final Elements options = td.select("select option");
		  if (options != null && options.size() > 0) {
			  team = (FiveDimesTeamPackage)parseTotalSelectOption(options, team, " ", null); }
		  else {
			  final String data = getHtmlFromLastIndex(td, ">");
			  team = (FiveDimesTeamPackage)parseTotalData(data, 0, " ", null, team);
		   }
	 ...
	 */
	private FiveDimesTeamPackage getTeamTotal(Element td, FiveDimesTeamPackage team) throws BatchException {
		...
		if(selectStr.length()>0) {
		  ...
		  final Elements options = td.select("select option");
		  if (options != null && options.size() > 0) {
			  team = (FiveDimesTeamPackage)parseTeamTotalSelectOption(options, team, " ", null); }
		  else {
			  final String data = getHtmlFromLastIndex(td, ">");
			  team = (FiveDimesTeamPackage)parseTeamTotalData(data, 0, " ", null, team);
		   }
	 ...
	 * newly added method on 31-10-18
	 */
	private FiveDimesTeamPackage getTeamTotalSecond(Element td, FiveDimesTeamPackage team) throws BatchException {
		...
		if(selectStr.length()>0) {
		  ...
		  final Elements options = td.select("select option");
		  if (options != null && options.size() > 0) {
			  team = (FiveDimesTeamPackage)parseTeamTotalSelectOptionSecond(options, team, " ", null); }
		  else {
			  final String data = getHtmlFromLastIndex(td, ">");
			  team = (FiveDimesTeamPackage)parseTeamTotalDataSecond(data, 0, " ", null, team);
		   }
