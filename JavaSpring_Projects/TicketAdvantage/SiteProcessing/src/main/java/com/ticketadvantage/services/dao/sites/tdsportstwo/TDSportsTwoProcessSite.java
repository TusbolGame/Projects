 ...
 *
 */
public class TDSportsTwoProcessSite extends TDSportsProcessSite {
	private static final Logger LOGGER = Logger.getLogger(TDSportsTwoProcessSite.class);
	protected TDSportsTwoParser BSP = new TDSportsTwoParser();

	/**
	 * 
	 */
	public TDSportsTwoProcessSite(String host, String username, String password, boolean isMobile,
			boolean showRequestResponse) {
		super("TDSportsTwo", host, username, password, isMobile, showRequestResponse);
		LOGGER.info("Entering TDSportsTwoProcessSite()");
		...
		LOGGER.info("Exiting TDSportsTwoProcessSite()");
	}
...
//				{ "http://www.508bet.net/", "ML02", "yellow", "100", "100", "100", "Baltimore", "ET"}
			...
			for (int i = 0; i < TDSITES.length; i++) {
				final TDSportsTwoProcessSite processSite = new TDSportsTwoProcessSite(TDSITES[i][0], TDSITES[i][1],
