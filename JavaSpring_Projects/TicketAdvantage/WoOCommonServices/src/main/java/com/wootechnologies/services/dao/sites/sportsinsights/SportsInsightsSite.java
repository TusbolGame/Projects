...
import com.wootechnologies.model.ClosingLine;
...
import com.wootechnologies.model.SportsInsightsEvent;
...
//			}
			...
			final EventPackage ep = processSite.getEventByIdNoSport("514");
			LOGGER.error("EventPackage: " + ep);
...
/*
			Set<EventPackage> eps = processSite.forceGetAllTodayOnEvents();
			for (EventPackage ep : eps) {
				LOGGER.error("ep.getId(): " + ep.getId());
...
//			LOGGER.debug("MLB EventsPackage: " + ep);
	...
	public EventPackage getEventByIdNoSport(String rotationId) {
		...
		try {
			...
			List<NameValuePair> retValue = httpClientWrapper.getJSONSite("https://sportsinsights.actionnetwork.com/live-odds", null, setupHeader(false));
			String json = httpClientWrapper.getCookiesAndXhtml(retValue);
			LOGGER.debug("json: " + json);
			String userId = sip.getUserId(json);
			
			retValue = httpClientWrapper.getJSONSite("https://sportsinsights.actionnetwork.com/" + SETUP_URL + userId, null, setupHeader(false));
	...
	public SportsInsightsEvent getEventByIdAll(String rotationId, String gameSport, Integer day) {
		...
		try {
			...
			final List<NameValuePair> retValue = httpClientWrapper.getJSONSite("https://sportsinsights.actionnetwork.com/" + SETUP_URL, null, setupHeader(false));
			final String json = httpClientWrapper.getCookiesAndXhtml(retValue);
...
//		LOGGER.debug("EventPackages: " + eps);
	...
	public ClosingLine getClosingLine(Integer sportsInsightsEventId, Integer lineType, Integer gameType, Integer sportId) throws BatchException {
		...
		try {
			...
			LOGGER.info("url: " + url);
			List<NameValuePair> retValue = httpClientWrapper.getJSONSite(url, null, setupHeader(false));
			String json = httpClientWrapper.getCookiesAndXhtml(retValue);
	...
	public String loginToSite(String username, String password) throws BatchException {
		...
		String data1 = "{\"email\":\"mojaxsventures@gmail.com\",\"password\":\"action1\",\"remember\":true}";
		this.httpClientWrapper.postJSONSite("https://sportsinsights.actionnetwork.com/api/account/login/", null, data1, headerValuePairs);
...
//		LOGGER.debug("ep: " + ep);
	...
	private String commonSetup() throws BatchException {
		...
		// Authenticate first
		loginToSite(this.httpClientWrapper.getUsername(), this.httpClientWrapper.getPassword());
		List<NameValuePair> retValue = httpClientWrapper.getJSONSite("https://sportsinsights.actionnetwork.com/" + SETUP_URL, null, setupHeader(false));
		String json = httpClientWrapper.getCookiesAndXhtml(retValue);
