      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="card noprint">
            <div class="card-body">
              <form id="form" action="{{route('invoice.getreport')}}" method="POST">
                @csrf()
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      <label for="">@lang('main.selectRecipient')</label>
                      <select name="supplier_name" id="customer-selector"
                              class="js-example-basic-single form-control">
                        <option value="all">@lang('main.all')</option>
                        @foreach (@$customer_info as $customerItem)
                          @if (@$customer_name == $customerItem)
                            <option value="{{$customerItem}}" selected>{{@$customerItem}}</option>
                          @else
                            <option value="{{$customerItem}}">{{@$customerItem}}</option>
                          @endif
                        @endforeach
                      </select>
                    </div>
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="form-group">
                      <button type="submit" class="mr-1 ml-1 md-1 btn btn-primary"><i
                            class="fa fa-search">&nbsp;</i> @lang('main.search')
                      </button>

                      <a class="btn btn-success mr-1 ml-1 md-1" href="{{url('/invoice')}}" id="all-button">
                        <i class="fas fa-plus-square">&nbsp; </i> Tutti Invoices</a>

                      <a href="#" class="btn btn-danger mr-1 ml-1 md-1" id="email-button">
                        <i class="fa fa-envelope">&nbsp;</i>
                        Send Unpaid Lists Email</a>
                    </div>
                  </div>
                </div>
              ...
              @foreach ($invoices as $invoice)
                <tr>
                  ...
                  <td>
                    ...
                    @if (@$invoice->payment_status == '1')
                      <span class="badge badge-success">@lang('main.toPaid')</span>
                    @else
                      <span class="badge badge-danger">@lang('main.paid')</span>
                    @endif

                  </td>
                  <td>
                    <a href="{{route('invoice.show', $invoice)}}"
                       class="btn btn-primary  btn-sm">@lang('main.sendMail')</a>

                    <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                            data-target="#previeworder{{@$invoice->order->id}}"
                            data-whatever="@mdo">@lang('main.preview')
                    </button>
                    <button onclick="printDiv('printableArea')" id="printButton"
