...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Start()
        {
            ...
            level = GameObject.Find("Main Camera").GetComponent<ImagePanelControl>().level;
            if (Validate())
            {
                Setup(true);
                answer = Camera.main.GetComponent<ImagePanelControl>().answer[CurrentPanel + 1];
                Debug.Log(answer);

                InvokeRepeating("GoToNextPanel", 6f + 4.0f + 10f - (level - 1f), 10f - (level - 1f));
        ...
        private void Update()
        {
            if (NumberOfPanels == 0 || !gameStart) return;
        ...
        {
            int[] dir_numbers;
            dir_numbers = Camera.main.GetComponent<ImagePanelControl>().dir_numbers;
            int[] imageNumbers;

            if (this.transform.name == "TrueImage")
            {
                imageNumbers = Camera.main.GetComponent<ImagePanelControl>().trueList;
            }
            else
            {
                imageNumbers = Camera.main.GetComponent<ImagePanelControl>().falseList;
            }

            GameObject image_panel = this.transform.GetChild(0).transform.GetChild(0).transform.GetChild(0).gameObject;
            
            for(int i = 1; i < 19; i++)
            {
                GameObject image_obj = Instantiate(image_panel);
                int imageNumber = UnityEngine.Random.Range(1, 3);
                //Sprite image = Resources.Load<Sprite>("Images/CompareImages/" + dir_numbers[i].ToString() + "/" + imageNumber.ToString());
                Sprite image = Resources.Load<Sprite>("Images/CompareImages/" + dir_numbers[i].ToString() + "/" + imageNumbers[i].ToString());
                image_obj.transform.SetParent(this.transform.GetChild(0).transform.GetChild(0));
                image_obj.GetComponent<Image>().sprite = image;
                image_obj.transform.name = imageNumber.ToString();
            }
            int _imageNumber = imageNumbers[0];
            Sprite _image = Resources.Load<Sprite>("Images/CompareImages/" + dir_numbers[0].ToString() + "/" + _imageNumber.ToString());
            image_panel.GetComponent<Image>().sprite = _image;
            image_panel.transform.name = _imageNumber.ToString();
        ...
        public void GoToNextPanel()
        {
            ...
            if (nearestPanel != (NumberOfPanels - 1))
            {
                ...
                Camera.main.GetComponent<ImagePanelControl>().trueAnswer = answer;
                Debug.Log(answer);
            }
            else
            {
