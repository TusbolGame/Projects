...
public class VodMenuEscolhaFilmes extends AppCompatActivity implements OnClickListener {
    private static final int TIME_INTERVAL = 2000;
    @BindView(R.id.menu_vod_mov_lista)
    LinearLayout menu_vod_mov_lista;
    @BindView(R.id.menu_vod_mov_app)
    LinearLayout menu_vod_mov_app;
    ...
    public void AbrirIPTVCORE(String reideus)
    {
        try {
            ...
            String mode = "vod";
            String tvqrqu_url = "http://mikkm.xyz/android/androidApi.php?url="+dns+"&username=" + username + "&password=" + password + "&mode="+mode;
            intent.setData(Uri.parse(tvqrqu_url));

            intent.putExtra("preferred_player_package", mSessionManager.getPackageSelectedTV());
    ...
    public void showIptvCoreNotFoundDialog() {
        ...
        builder2.setTitle(mSessionManager.getNameAPP()+"-Erro");
        builder2.setMessage("Instalar por favor o IPTV-Core na playstore. E volte de novo aqui.");
        builder2.setPositiveButton("Ok, Instalar Agora.",
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        try {
                            context.startActivity(new Intent("android.intent.action.VIEW", Uri.parse("market://details?id=ru.iptvremote.android.iptv.core")));
                        } catch (ActivityNotFoundException e) {
                            context.startActivity(new Intent("android.intent.action.VIEW", Uri.parse("http://play.google.com/store/apps/details?id=ru.iptvremote.android.iptv.core")));
                        }
                    }
                });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.vod_menu_vod_mov);
        ButterKnife.bind(this);
        changeStatusBarColor();
        hideSystemUi();
        makeButtonsClickable();
        this.menu_vod_mov_lista.setOnFocusChangeListener(new OnFocusChangeAccountListener(this.menu_vod_mov_lista));
        this.menu_vod_mov_app.setOnFocusChangeListener(new OnFocusChangeAccountListener(this.menu_vod_mov_app));
        this.menu_vod_mov_lista.requestFocus();
    ...
    private void makeButtonsClickable() {
        this.menu_vod_mov_lista.setOnClickListener(this);
        this.menu_vod_mov_app.setOnClickListener(this);
    }

    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.menu_vod_mov_lista:
                if(mSessionManager.getSeries().equals("true") || mSessionManager.getSeries().equals("nao")){
                    ...
                    AbrirIPTVCORE("normal");
                }
                overridePendingTransition(R.anim.fade_out, R.anim.fade_in);
                return;
            case R.id.menu_vod_mov_app:
