...
public class EPGUtil {
    ...
    static class C20991 implements Picasso.Listener {
        C20991() {
        }
    ...
    public static void loadImageInto(Context context, String url, int width, int height, Target target) {
        ...
        if (url == null || url.equals("")) {
            picasso.load(R.drawable.logo).into(target);
        } else {
    ...
    private static void initPicasso(Context context) {
        if (picasso == null) {
            picasso = new Builder(context).downloader(new OkHttpDownloader(new OkHttpClient())).listener(new C20991()).build();
        }
    }
}
