...
public class NSTPlayerArchive {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 3:
                    ...
                    return;
                case 4:
                    NSTPlayerArchive.this.f36$.id(R.id.app_video_volume_box).gone();
                    NSTPlayerArchive.this.f36$.id(R.id.app_video_brightness_box).gone();
                    NSTPlayerArchive.this.f36$.id(R.id.app_video_fastForward_box).gone();
                    return;
                default:
                    return;
            }
    ...
    private final OnSeekBarChangeListener mSeekListener = new C20286();
    private long newPosition = -1;
    private final OnClickListener onClickListener = new C20231();
    public OnControlPanelVisibilityChangeListener onControlPanelVisibilityChangeListener = new C20275();
    private OnErrorListener onErrorListener = new C20242();
    private OnInfoListener onInfoListener = new C20264();
    private Runnable oncomplete = new C20253();
    ...
    class C20231 implements OnClickListener {
        C20231() {
        }
    ...
    class C20242 implements OnErrorListener {
        C20242() {
        }
    ...
    class C20253 implements Runnable {
        C20253() {
        }
    ...
    class C20264 implements OnInfoListener {
        C20264() {
        }
    ...
    class C20275 implements OnControlPanelVisibilityChangeListener {
        C20275() {
        }
    ...
    class C20286 implements OnSeekBarChangeListener {
        C20286() {
        }

        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                NSTPlayerArchive.this.f36$.id(R.id.app_video_status).gone();
                ...
                NSTPlayerArchive.this.f36$.id(R.id.app_video_currentTime).text(time);
            }
        }

        public void onStartTrackingTouch(SeekBar seekBar) {
            NSTPlayerArchive.this.isDragging = true;
            NSTPlayerArchive.this.show(3600000);
    ...
    class C20308 implements IMediaPlayer.OnCompletionListener {
        C20308() {
        }
    ...
    class C20319 implements IMediaPlayer.OnErrorListener {
        C20319() {
        }
    ...
    public void setTitle(CharSequence title) {
        this.f36$.id(R.id.app_video_title).text(title);
    }

    public void doPauseResume() {
        if (this.status == this.STATUS_COMPLETED) {
            this.f36$.id(R.id.app_video_replay).gone();
    ...
    private void updatePausePlay() {
        if (this.videoView.isPlaying()) {
            this.f36$.id(R.id.exo_play).gone();
            this.f36$.id(R.id.exo_pause).visible();
            return;
        }
        this.f36$.id(R.id.exo_pause).gone();
        this.f36$.id(R.id.exo_play).visible();
    }
    ...
    public void show(int timeout) {
        if (!this.isShowing) {
            this.f36$.id(R.id.app_video_top_box).visible();
            this.f36$.id(R.id.app_video_bottom_box).visible();
            this.f36$.id(R.id.controls).visible();
            if (!this.isLive) {
                showBottomControl(true);
            }
            if (!this.fullScreenOnly) {
                this.f36$.id(R.id.app_video_fullscreen).visible();
            }
            this.isShowing = true;
    ...
    public void showBottomControl(boolean show) {
        int i;
        int i2 = 0;
        Query id = this.f36$.id(R.id.app_video_currentTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        id = this.f36$.id(R.id.app_video_endTime);
        if (show) {
            i = 0;
        } else {
            i = 8;
        ...
        id.visibility(i);
        Query id2 = this.f36$.id(R.id.app_video_seekBar);
        if (!show) {
            i2 = 8;
        }
        id2.visibility(i2);
    ...
    public NSTPlayerArchive(final Activity activity) {
        ...
        this.f36$ = new Query(activity);
        this.videoView = (IjkVideoView) activity.findViewById(R.id.video_view);
        this.videoView.setOnCompletionListener(new C20308());
        this.videoView.setOnErrorListener(new C20319());
        ...
        this.f36$.id(R.id.app_video_fullscreen).clicked(this.onClickListener);
        this.f36$.id(R.id.app_video_finish).clicked(this.onClickListener);
        this.f36$.id(R.id.app_video_replay_icon).clicked(this.onClickListener);
    ...
    private void statusChange(int newStatus) {
        ...
        if (!this.isLive && newStatus == this.STATUS_COMPLETED) {
            ...
            hideAll();
            this.f36$.id(R.id.app_video_replay).visible();
            ...
            hideAll();
            this.f36$.id(R.id.app_video_loading).visible();
    ...
    private void hideAll() {
        this.f36$.id(R.id.app_video_replay).gone();
        this.f36$.id(R.id.app_video_top_box).gone();
        this.f36$.id(R.id.app_video_bottom_box).gone();
        this.f36$.id(R.id.controls).gone();
        this.f36$.id(R.id.app_video_loading).gone();
        this.f36$.id(R.id.app_video_fullscreen).invisible();
        this.f36$.id(R.id.app_video_status).gone();
        ...
        if (this.videoView != null && !this.fullScreenOnly) {
            this.handler.post(new Runnable() {
                public void run() {
                    ...
                    if (portrait) {
                        NSTPlayerArchive.this.f36$.id(R.id.app_video_box).height(NSTPlayerArchive.this.initHeight, false);
                    } else {
                        NSTPlayerArchive.this.f36$.id(R.id.app_video_box).height(Math.min(NSTPlayerArchive.this.activity.getResources().getDisplayMetrics().heightPixels, NSTPlayerArchive.this.activity.getResources().getDisplayMetrics().widthPixels), false);
                    }
    ...
    private void showStatus(String statusText) {
        this.f36$.id(R.id.app_video_status).visible();
        this.f36$.id(R.id.app_video_status_text).text(statusText);
    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        this.f36$.id(R.id.app_video_volume_icon).image(i == 0 ? R.drawable.ic_volume_off_white_36dp : R.drawable.ic_volume_up_white_36dp);
        this.f36$.id(R.id.app_video_brightness_box).gone();
        this.f36$.id(R.id.app_video_volume_box).visible();
        this.f36$.id(R.id.app_video_volume_box).visible();
        this.f36$.id(R.id.app_video_volume).text(s).visible();
    }
    ...
    private void onProgressSlide(float percent) {
        ...
        if (showDelta != 0) {
            this.f36$.id(R.id.app_video_fastForward_box).visible();
            this.f36$.id(R.id.app_video_fastForward).text((showDelta > 0 ? "+" + showDelta : "" + showDelta) + "s");
            this.f36$.id(R.id.app_video_fastForward_target).text(generateTime(this.newPosition) + "/");
            this.f36$.id(R.id.app_video_fastForward_all).text(generateTime(duration));
        }
    }
    ...
    private void onBrightnessSlide(float percent) {
        ...
        this.f36$.id(R.id.app_video_brightness_box).visible();
        ...
        this.f36$.id(R.id.app_video_brightness).text(((int) (lpa.screenBrightness * 100.0f)) + "%");
    ...
    private long setProgress() {
        ...
        this.f36$.id(R.id.app_video_currentTime).text(generateTime(position));
        this.f36$.id(R.id.app_video_endTime).text(generateTime(this.duration));
        return position;
    }
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == 0) {
            this.f36$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_exit_white_36dp);
        } else {
            this.f36$.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_white_24dp);
        }
    }
    ...
    public void setShowNavIcon(boolean show) {
        this.f36$.id(R.id.app_video_finish).visibility(show ? 0 : 8);
    }
    ...
    public NSTPlayerArchive fullScreenRatio() {
